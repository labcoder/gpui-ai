...
<body${bodyClass ? ` class="${bodyClass}"` : ""}>
  ...
  ${rail}
  ${body}
  <footer><span>Field notes for AI interfaces in Rust.</span><nav aria-label="Repository files"><a href="https://github.com/labcoder/gpui-ai/blob/main/README.md">README</a><a href="https://github.com/labcoder/gpui-ai/blob/main/Cargo.toml">Cargo.toml</a><a href="https://github.com/labcoder/gpui-ai/blob/main/site/README.md">Site guide</a></nav><span>24 components · 3 themes · one WASM gallery</span></footer>
</body>
</html>`;
}
...
export function homePage() {
  ...
  return documentShell({
    ...
    body: `<main id="content" class="home" tabindex="-1">
      ...
      <dl class="build-metadata" aria-label="Build metadata"><div><dt>Surface</dt><dd>24 stable stories</dd></div><div><dt>Artifact</dt><dd>One shared release WASM</dd></div><div><dt>Distribution</dt><dd>Not published · source checkout</dd></div><div><dt>Build</dt><dd>Static local production output</dd></div></dl>
      ...
      <section class="architecture-strip" aria-labelledby="architecture-title"><div><p class="eyebrow">Install / current status</p><h2 id="architecture-title">Build from source.</h2><p>mighty-gpui is not published yet. Clone the repository and keep the paired GPUI revisions pinned by <code>Cargo.lock</code>.</p><pre><code>git clone https://github.com/labcoder/gpui-ai\ncd gpui-ai\ncargo check --workspace</code></pre></div><dl><div><dt>Page chrome</dt><dd>Page chrome: semantic HTML, CSS, and browser JavaScript.</dd></div><div><dt>Live examples</dt><dd>One shared Rust/WASM gallery; never one binary per page.</dd></div><div><dt>Ownership</dt><dd>Applications own durable data and async work; components report typed intent.</dd></div></dl></section>
      <section class="index-preview" aria-labelledby="preview-title"><p class="eyebrow">Component index</p><h2 id="preview-title">A working vocabulary, not a screenshot pack.</h2><div class="catalog-search"><label for="catalog-search">Find a pattern</label><input id="catalog-search" type="search" data-catalog-search autocomplete="off" placeholder="Search by name, category, or purpose"><p data-catalog-status role="status" aria-live="polite">24 components</p></div>${catalogGroups("components/")}</section>
    </main>`,
  });
}
...
export function catalogPage() {
  return documentShell({
    ...
    body: `<main id="content" class="catalog-page" tabindex="-1">
      ...
      <div class="catalog-search"><label for="catalog-search">Find a pattern</label><input id="catalog-search" type="search" data-catalog-search autocomplete="off" placeholder="Search by name, category, or purpose"><p data-catalog-status role="status" aria-live="polite">24 components</p></div>
      ${catalogGroups()}
    </main>`,
  });
}
