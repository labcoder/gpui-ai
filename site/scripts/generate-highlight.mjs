...
// The same code, split the way the pages load it. The combined file above
// stays for tools that read it from disk, but the app no longer bundles it:
// every route paid ~313 KB of other pages' code through one static import.
// Each component page now owns one chunk with its raw and highlighted
// variants; the documentation samples share one chunk; the install lines stay
// eager because the home page shows them in its first paint.
const CODE = join(GENERATED, "code");
mkdirSync(CODE, { recursive: true });
const written = new Set();
const emit = (name, payload) => {
  written.add(`${name}.json`);
  writeFileSync(join(CODE, `${name}.json`), `${JSON.stringify(payload, null, 2)}\n`);
};
const CHUNK_COMMENT =
  "Generated by site/scripts/generate-highlight.mjs. Do not edit. Loaded per route by site/app/code.ts.";
for (const [slug, variants] of Object.entries(highlighted)) {
  emit(slug, { $comment: CHUNK_COMMENT, raw: snippets[slug], lines: variants });
}
const { install: installSample, ...documentationSamples } = extras;
emit("samples", { $comment: CHUNK_COMMENT, samples: documentationSamples });
// A slug that leaves the catalog must take its chunk with it: the directory
// is committed, and a stale file would keep a deleted component importable.
for (const name of readdirSync(CODE)) {
  if (name.endsWith(".json") && !written.has(name)) {
    rmSync(join(CODE, name));
  }
}
writeFileSync(
  join(GENERATED, "install.json"),
  `${JSON.stringify({ $comment: CHUNK_COMMENT, ...installSample }, null, 2)}\n`,
);
...
process.stdout.write(
  `site/generated/highlight.json: ${Object.keys(highlighted).length} snippets, ` +
    ...
    `site/generated/code: ${written.size} route chunks + install.json\n`,
);
